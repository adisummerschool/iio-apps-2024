...
int main() {
    ...
    for (int value = 1; value <= 10; value++) {
        ...
        if (iio_channel_attr_write(channel, ATTRIBUTE_NAME, buf) < 0) {
            perror("Writing attribute failed");
            return -1;
        } else printf("Set %s to %d\n", ATTRIBUTE_NAME, value);

        iio_channel_attr_read(channel, ATTRIBUTE_NAME, buf, sizeof(buf));
        if (iio_channel_attr_read(channel, ATTRIBUTE_NAME, buf, sizeof(buf)) < 0) {
            perror("Reading attribute failed");
            return -1;
        } else printf("Read %s: %s\n", ATTRIBUTE_NAME, buf);
